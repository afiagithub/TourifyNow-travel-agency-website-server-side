...
// middlewares
app.use(cors({
  origin: ["http://localhost:5173", "https://tourify-now-app.web.app", "https://afia-assignment-10-server.vercel.app"],
  credentials: true
}))
...
async function run() {
  try {
    ...
    app.get("/sortedSpot", async (req, res) => {
      const cursor = touristSpotCollection.find().sort({ average_cost: 1 });
      const result = await cursor.toArray();
      res.send(result)
    })
    ...
    app.get("/countryList/:name", async (req, res) => {
      const query = { country_Name: req.params.name };
      const result = await touristSpotCollection.find(query).toArray();
      res.send(result)
    })
